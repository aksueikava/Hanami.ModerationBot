import { DMChannel, NewsChannel, TextChannel, Message, User, MessageEmbed } from 'discord.js'
...
export default class Pages {
  ...
  send(channel: TextChannel | NewsChannel | DMChannel, user: User) {
    return channel.send(new MessageEmbed(this.pages[this.page - 1])).then(async msg => {
      this.message = msg

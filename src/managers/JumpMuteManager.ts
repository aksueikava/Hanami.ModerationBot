...
import { JumpMute, Punishment } from '../utils/db'

export interface JumpMuteData {
  userID: string
  moderID: string
  guildID?: string
  duration?: number
...
export default class JumpMuteManager {
  static check(interval: number = config.ticks.checkInterval) {
    JumpMute.filter(d => d.endTick && d.endTick - Date.now() < interval)
  ...
  static async mute(data: JumpMuteData) {
    ...
    const [muteDoc, targetMember] = await Promise.all([
      JumpMute.getOne({ userID: data.userID }),
    ...
    if (typeof data.duration === 'number') {
      ...
      if (data.duration < config.ticks.checkInterval) {
        ...
        setTimeout(
          JumpMuteManager.unmute.bind(null, unmuteOptions),
          data.duration
        )
      }
    }
    ...
    Punishment.insertOne({
      ...
      reason,
      type: config.ids.punishments.jumpmute,
      tick
    })
    ...
    if (logsCh) {
      ...
      const embed = {
        ...
        title: `JumpMute${moderUser ? ` | ${moderUser.tag}` : ''}`,
        description: `**Причина**\n>>> ${reason}`,
        fields: [
          {
            name: 'Пользовать',
    ...
    if (targetMember) targetMember.roles.add(config.ids.roles.jumpmute)
  }
  ...
  static async unmute(data: ChatUnmuteData) {
    ...
    JumpMute.deleteOne({ userID: data.userID })
    ...
    if (logsCh) {
      ...
      const embed = {
        color: config.meta.defaultColor,
        title: `JumpUnmute${moderUser ? ` | ${moderUser.tag}` : ''}`,
        description: `**Причина**\n>>> ${reason}`,
        fields: [
          {
            name: 'Пользовать',
    ...
    if (targetMember) targetMember.roles.remove(config.ids.roles.jumpmute)
  }
}
